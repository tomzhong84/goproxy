...
func GetRandProxy() (string, error) {
	// 获取代理
	op := redis.ZRangeBy{
		Min:    MIN_SCORE, // 最小分数
		Max:    MAX_SCORE, // 最大分数
	...
	proxyInfo, err := rdb.ZRangeByScore("proxies:ranking", op).Result()
	if err != nil {
		// 出错尝试重新获取代理
	}
	fmt.Println("获取到代理总数", len(proxyInfo))
	proxy := proxyInfo[rand.Intn(len(proxyInfo))]
...
func NewProxyHttpServer() *ProxyHttpServer {
	...
	_proxy := func(_ *http.Request) (*url.URL, error) {
		proxyInfo, err := GetRandProxy()
		if err != nil {
			// 重新获取逻辑
		}
		fmt.Println("当前获取代理", proxyInfo)
		return url.Parse(proxyInfo)
	}
	...
	proxy := ProxyHttpServer{
		...
		Tr: &http.Transport{TLSClientConfig: tlsClientSkipVerify, Proxy: _proxy},
	}
